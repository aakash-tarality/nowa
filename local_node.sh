...
add_genesis_funds() {
  local keyname="$1"
  nowad genesis add-genesis-account "$keyname" 1000000000000000000000anowa --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for nowad.","denom_units":[{"denom":"anowa","exponent":0,"aliases":["attonowa"]},{"denom":"nowa","exponent":18,"aliases":[]}],"base":"anowa","display":"nowa","name":"Nowa Token","symbol":"NOWA","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"anowa",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  nowad genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000anowa --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  # --------- Finalize genesis ---------
  nowad genesis gentx "$VAL_KEY" 1000000000000000000000anowa --gas-prices ${BASEFEE}anowa --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
